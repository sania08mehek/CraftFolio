...
def build_portfolio(customization: dict[str, Any], user_data: dict[str, Any]) -> str:
    ...
    Full pipeline:
      ...
      3. Build navbar (always included)
      4. Loop sections, load layout, inject user data
      5. Assemble wrapper.html
    ...
    animations_js = anim_js_path.read_text(encoding="utf-8")

    # ── 3. Navbar (always first, special {{NAV_LINKS}} placeholder) ──────────
    section_layouts   = customization.get("section_layouts", {})
    excluded_sections = customization.get("excluded_sections", [])
    ...
    for section in sections_order:
        ...
        try:
            ...
            # Gracefully skip missing templates (log but don't crash)
    ...
    meta_description = (
        ...
        f"{user_data.get('bio', '')[:120]}..."
    )

    # Determine theme id for data-theme attribute (lowercase id)
    ...
    final_html = final_html.replace("{{FOOTER}}",           "")   # footer optional
